import { useTranslation } from "react-i18next";

const GITHUB_PATH = "M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12";

const CTA = () => {
  const { t } = useTranslation();

  return (
    <section
      className="py-28 text-center text-white relative overflow-hidden"
      style={{ background: "#2B3141" }}
    >
      {/* Glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[520px] h-[300px] rounded-full blur-3xl"
          style={{ background: "rgba(232,101,74,0.12)" }}
        />
      </div>

      <div className="relative z-10 max-w-2xl mx-auto px-6">
        <span
          className="text-xs font-semibold tracking-widest px-4 py-1.5 rounded-full"
          style={{ color: "#E8654A", background: "rgba(232,101,74,0.12)" }}
        >
          {t("cta_badge")}
        </span>

        <h2 className="text-4xl font-bold mt-6 mb-4 leading-tight">
          {t("cta_h1")}
          <br />
          {t("cta_h2")}
        </h2>

        <p className="mb-10 text-lg" style={{ color: "#9AA0B4" }}>
          {t("cta_sub")}
        </p>

        <div className="flex gap-4 justify-center flex-wrap">
          {/* Primary coral */}
          <button
            className="flex items-center gap-2.5 font-semibold px-8 py-4 rounded-xl transition text-white"
            style={{ background: "#E8654A" }}
            onMouseEnter={e => e.currentTarget.style.background = "#D4512F"}
            onMouseLeave={e => e.currentTarget.style.background = "#E8654A"}
            onMouseDown={e => e.currentTarget.style.background = "#C04020"}
            onMouseUp={e => e.currentTarget.style.background = "#D4512F"}
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white" xmlns="http://www.w3.org/2000/svg">
              <path d={GITHUB_PATH} />
            </svg>
            {t("hero_github")}
          </button>

          {/* Ghost */}
          <button
            className="font-semibold px-8 py-4 rounded-xl transition backdrop-blur-sm"
            style={{
              background: "rgba(255,255,255,0.08)",
              border: "1.5px solid rgba(255,255,255,0.18)",
              color: "#FFFFFF",
            }}
            onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.14)"}
            onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.08)"}
          >
            {t("view_docs")}
          </button>
        </div>
      </div>
    </section>
  );
};

export default CTA;
