...
const TaskCompletionModal = ({ project, modalState, onClose, onSubmit, C }) => {
  ...
  useEffect(() => {
    ...
    getProjectTaskCommits(project._id)
      .then((data) => { if (!ignore) setCommits(data.commits || []); })
      .catch((err) => { if (!ignore) setError(err?.response?.data?.message || "Commitlarni yuklab bo'lmadi"); })
  ...
  const availableCommits = commits.filter((c) => !c.task || c.task?._id === task._id);
  const linkedCommits = commits.filter((c) => c.task && c.task?._id !== task._id);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b"
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: C.placeholder }}>
              Task completion
            </p>
            ...
            >
              {members.map((member) => (
                <option
                  ...
                  style={{ backgroundColor: C.inputBg, color: C.heading }}
                >
            ...
            <div className="flex items-center justify-between gap-3 mb-3">
              <div>
                ...
                <p className="text-sm" style={{ color: C.muted }}>
                  Commit tanlashingiz yoki o'tkazib yuborishingiz mumkin.
                </p>
              </div>
              <button
                type="button"
                ...
                disabled={submitting}
                className="text-xs font-semibold px-3 py-2 rounded-lg transition-all shrink-0"
                style={{
                  backgroundColor: C.cardBg, color: C.muted,
                  border: `1px solid ${C.borderSubtle}`, opacity: submitting ? 0.6 : 1,
                }}
              >
                O'tkazib yuborish
              </button>
